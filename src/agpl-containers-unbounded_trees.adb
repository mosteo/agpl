...
--     end Image;
   ...
   --  In body, so it is not primitive for Cursor

   function "+" (This : Cursor'Class) return Cursor is
   begin
      return Cursor (This);
   end "+";
   ...
   function Equivalent (L, R : Cursor) return Boolean is
   begin
      return L.Ref.Current = R.Ref.Current;
   end Equivalent;
   ...
   function Find (From : Cursor; Data : Node_Data) return Cursor is
      procedure Find (From : Cursor; Found : out Cursor) is
      begin
         if not From.Is_Valid then
            raise Constraint_Error with "Cursor is null";
         elsif not From.Has_Element then
            raise Constraint_Error with "Cursor has no data";
         elsif From.Query.all = Data then
            Found := From;
         else
            declare
               procedure Find (I : Cursor) is
               begin
                  Find (I, Found);
               end Find;
            begin
               From.Iterate_Children (Find'Access);
            end;
         end if;
      end Find;
   ...
   begin
      Find (From, Found);
      if Found.Has_Element then
         return Found;
      else
         raise Constraint_Error with "Data not found";
      end if;
   end Find;
   ...
   function Generic_Find (From : Cursor; Data : Node_Data) return Cursor is
      procedure Find (From : Cursor; Found : out Cursor) is
      begin
         if not From.Is_Valid then
            raise Constraint_Error with "Cursor is null";
   ...
   function Child_Count (This : Cursor) return Natural is
   begin
      return Natural (This.Ref.Current.Children.Length);
   end Child_Count;
      ...
      begin
         if This /= null then
            if This.Is_Root then
               Log (Prefix & "@ - " & Image (+This.Data), Always, Log_Section);
            else
               Log (Prefix &
                    Image (+This.Index) & " - " &
                    Image (+This.Data), Always, Log_Section);
            end if;
   ...
   begin
      if This.Root /= null then
         ...
         Log ("NULL TREE", Always, Log_Section);
      end if;
   end Print;
...
--           Print_Debug (Tree (Pos.Root.all));
   ...
   procedure Adjust (This : in out Tree) is
      Src : constant Tree_Access := This.This;
   begin
      This.This := This'Unchecked_Access;
      This.Root := null;

      Copy (Src.Get_Root, This);
   end Adjust;
