...
package body Agpl.Strings is
   ...
   function Replace (S, Pattern, New_Pattern : String) return String is
      Result : String (S'First .. S'First + S'Length * New_Pattern'Length + S'Length);
      --  Worst case: all replaced or nothing replaced with New_Pattern = ""

      Pos    : Positive := Result'First;
      Wold   : constant Natural := Pattern'Length - 1;
      Wnew   : constant Integer := New_Pattern'Length - 1;
      I      : Positive := S'First;
   begin
...
--        Put_Line ("New:" & Result);
   ...
   ---------
      ...
      is
      begin
         --  Done here on first call to avoid non-preelaborable
         if Max_Length = 0 then
            for X in Enum'Range loop
               Max_Length := Natural'Max (Max_Length, X'Img'Length);
            end loop;
         end if;

         case Align is
            when Left =>
               return Lpad (X'Img, Max_Length + Extra_Pad, Fill);
            when Right =>
               return Rpad (X'Img, Max_Length + Extra_Pad, Fill);
         end case;
      end Pad;
