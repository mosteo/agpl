...
package body Agpl.Optimization.Concorde is
   ...
   begin
      Log ("At Solve_TSP", Debug, Log_Section);
...
--                 Print_Solution (New_Cost, Start, Result, No_Return => False);
   ...
   begin
      Log ("At Solve_ATSP", Debug, Log_Section);
...
--                 New_Line;
   ...
   is
      ...
      New_Cost : Cost_Matrix :=
                   Create (First_Row => Cost.First_Row,
                           ...
                           Default   => Inf);
   begin
      Log ("At Solve_MTSP", Debug, Log_Section);
      if Cost.First_Row /= 1 or else Cost.First_Col /= 1 or else Start'First /= 1 then
         raise Constraint_Error;
      end if;
      pragma Assert (Cost.Rows = Cost.Cols);
      ...
      if Start'Length = 1 and then not No_Return then
         Log ("Problem is single closed salesman, skipping transformation...",
              Debug, Log_Section);
   ...
   is
      ...
      --  Big_M : constant Costs := Get_Big_M (Cost);
   begin
      Log ("At Solve_MTSP_No_Return", Debug, Log_Section);
...
end Agpl.Optimization.Concorde;
