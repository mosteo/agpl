...
package body Agpl.Counter.Multi is
   ...
   protected body Safe_Object is
      ...
      begin
         return Values.Element (Key).Val;
      end Val;

      function Val (Key : String; Default : Integer) return Integer is
         I : constant Counter_Map.Cursor := Values.Find (Key);
      begin
         if Counter_Map.Has_Element (I) then
            return Counter_Map.Element (I).Val;
         else
            return Default;
         end if;
      end Val;
   ...
   is
   begin
      return This.Safe.Val (Key, Default);
   end Val;
