...
--           Encoding);
   ...
   function Has_Attribute (Item : Node; Attr : String) return Boolean is
   begin
      return Get_Attribute (Item, Attr, "") /= "";
   end Has_Attribute;
   ...
   function Get_With_Attribute (Parent : Node;
                                Name   : String;
                                Attr   : String;
                                Attrv  : String) return Node
   is
      Children : constant Node_Vector := Get_All (Parent, Name);
   begin
      for I in Children.First_Index .. Children.Last_Index loop
         if Get_Attribute (Children.Element (I), Attr, Attrv & "x") = Attrv then
            return Children.Element (I);
         end if;
      end loop;
      raise Constraint_Error with "Get_With_Attribute: No matching child";
   end Get_With_Attribute;
