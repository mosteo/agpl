...
package Agpl.Containers.Bulk.Utils is
   ...
   function Find (L : Lists.List; K : Key_Type) return Lists.Cursor;
   function Find (L : Lists.List; K : Key_Type) return Element_Type;
   function Contains (L : Lists.List;
                      K : Key_Type)
                      return Boolean;
   generic
      with function Is_Target (I : Lists.Cursor) return Boolean;
   function Find_Target (L : Lists.List) return Lists.Cursor;
   --  Works in reverse, can be mixed with deletions from the list
   ...
   function Tail (L : Lists.List) return Lists.List;
   ...
   function "-" (L, R : Lists.List) return Lists.List;
   function Substract (L, R : Lists.List) return Lists.List renames "-";
