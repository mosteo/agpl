...
begin
   ...
   begin
      ...
      begin
         for I in S'Range loop
            City_Poses (S (I)).X := 0.0;
            City_Poses (S (I)).Y := Float (Nth) * 4.0;
            Nth := Nth + 1;
         end loop;
      end;
      ...
      --  Solve!
      declare
         Sol : constant Result_Matrix := Solve_MTSP (S, C, No_Return => True);
      begin
         print_Problem (C);
         for I in S'Range loop
            Put_Line ("Salesman" & I'Img & " starts at city" & S (I)'Img);
         end loop;
         Print_Solution (C, S, Sol, No_Return => True);
