...
--  Invoke with: <program> <cities> <travelers> <noreturn> [random seed]

procedure T012_mtsp is
   ...
   procedure Usage is
   begin
      Put_Line ("Usage: <program> <cities> <travelers> <noreturn> [random seed]");
   end Usage;
...
begin
   ...
   if Argument_Count = 3 then
      ...
      Reset (R, Integer'Value (Argument (4)));
   end if;

   N := Cities'Value (Argument (1));
   M := Salesmen'Value (Argument (2));
   No_Return := Boolean'Value (Argument (3));
   ...
   begin
      ...
      --  Solve!
      declare
         Sol : constant Result_Matrix := Solve_MTSP (S, C, No_Return);
      begin
         print_Problem (C);
         for I in S'Range loop
            Put_Line ("Salesman" & I'Img & " starts at city" & S (I)'Img);
         end loop;
         Print_Solution (C, S, Sol, No_Return);
         ...
         declare
            ...
            begin
               ...
               for Salesman in Sol'Range loop
                  declare
                     Tour : constant Result_Matrix :=
                              Normalize_Tour (S (Salesman), Sol);
                  begin
                     for City in Tour'First (2) .. Tour'Last (2) - 1 loop
                        Drawer_Figures.Draw_Segment
                          (City_Poses (Tour (1, City)).X,
                           City_Poses (Tour (1, City)).Y,
                           City_Poses (Tour (1, City + 1)).X,
                           City_Poses (Tour (1, City + 1)).Y,
                           Get_Color (Pal, Color_List (Integer (Salesman)).all),
                           Draw);
                     end loop;
                     if not No_Return then
                        Drawer_Figures.Draw_Segment
                          (City_Poses (Tour (1, Tour'Last (2))).X,
                           City_Poses (Tour (1, Tour'Last (2))).Y,
                           City_Poses (Tour (1, Tour'First (2))).X,
                           City_Poses (Tour (1, Tour'First (2))).Y,
